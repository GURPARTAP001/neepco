...
export default function MyAccountRightCard({ title, description, time }) {
    const theme = useTheme();
    const isMobile = useMediaQuery(theme.breakpoints.down('sm'));
    ...
    const cardStyles = {
        ...
        marginBottom: "20px",
        justifyContent: "space-between",
        height: isMobile ? 'auto' : '120px', // Increased height for better spacing
        background: "white",
        marginInline: "5px",
    ...
    const imageStyles = {
        ...
        height: isMobile ? 200 : '100%', // Defined height for mobile
        borderRadius: '10px',
    ...
    return (
        <Card sx={cardStyles}>
            ...
            <Box sx={contentBoxStyles}>
                <CardContent>
                    <Typography component="div" variant={isMobile ? 'h6' : 'h5'} gutterBottom>
                        {title}
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                        {description}
